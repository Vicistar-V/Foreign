...
REVOKE EXECUTE ON FUNCTION public.get_pending_balance(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_pending_balance(uuid) TO authenticated, service_role;
...
BEGIN
  ...
  VALUES (_user_id,'pending'::wallet_type,-v_payable,'task_unlock'::transaction_type,
    'Pending consumed for machine payout','completed'::transaction_status,
...
REVOKE EXECUTE ON FUNCTION public.consume_pending_for_cycle(uuid, numeric, uuid, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.consume_pending_for_cycle(uuid, numeric, uuid, uuid) TO service_role;

DROP FUNCTION IF EXISTS public.promote_pending_to_withdrawable(uuid);
ALTER TABLE public.cached_balances DROP COLUMN IF EXISTS pending_balance;
...
BEGIN
  ...
  VALUES (_user_id,'pending'::wallet_type, naira_per_batch_for_user, 'task_earning'::transaction_type,
    'Daily task batch reward','completed'::transaction_status,
...
BEGIN
  ...
  RETURN jsonb_build_object('success',true,'today',today_utc,'task_enabled',cfg.task_enabled,
    ...
    'machine_count',machine_count,'total_batches_today',total_today,
...
-- Kill system treasury
DROP TRIGGER IF EXISTS protect_system_treasury ON public.profiles;
DROP FUNCTION IF EXISTS public.prevent_system_treasury_deletion() CASCADE;

DELETE FROM public.transactions WHERE user_id = '00000000-0000-0000-0000-000000000000';
DELETE FROM public.cached_balances WHERE user_id = '00000000-0000-0000-0000-000000000000';
