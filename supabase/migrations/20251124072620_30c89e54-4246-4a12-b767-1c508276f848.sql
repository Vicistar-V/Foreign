-- ============================================
-- NIGERIAN TIMEZONE FIX: Database Schema Updates
-- This migration ensures all date operations use Nigerian time (Africa/Lagos)
-- ============================================

-- 1. Update can_join_drop function to accept Nigerian date parameter
CREATE OR REPLACE FUNCTION public.can_join_drop(
  _user_id uuid,
...
BEGIN
  ...
  -- UPDATED: Check if already joined (using passed Nigerian date)
  SELECT EXISTS(
...
-- 2. Remove database defaults from date columns
-- Edge functions will now explicitly pass Nigerian dates
ALTER TABLE public.drop_entries 
ALTER COLUMN drop_date DROP DEFAULT;

COMMENT ON COLUMN public.drop_entries.drop_date IS 
'Nigerian business date (Africa/Lagos timezone). Always set explicitly by edge functions.';

ALTER TABLE public.daily_drop_logs 
ALTER COLUMN drop_date DROP DEFAULT;

COMMENT ON COLUMN public.daily_drop_logs.drop_date IS 
'Nigerian business date (Africa/Lagos timezone). Always set explicitly by edge functions.';
