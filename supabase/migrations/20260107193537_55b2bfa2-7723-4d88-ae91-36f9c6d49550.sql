-- Backfill balanced transactions for Victor's Machine 3 and Vivian's Machine 2
-- These machines were created without proper ledger entries

-- Victor (Machine 3) - Credit genesis bonus
...
SELECT 
  ...
  'genesis_bonus',
  'deposit',
  'Genesis Complete! Bonus credit for Machine 3 (Backfill)',
  'completed',
  json_build_object('backfill', true, 'reason', 'Machine created without ledger entry')
FROM profiles WHERE full_name = 'Victor';

-- Victor (Machine 3) - Debit for machine purchase
INSERT INTO transactions (user_id, amount, transaction_type, wallet_type, description, status, metadata)
SELECT 
  id,
  -1000,
  'drop_entry',
  'deposit',
  'Bought Machine 3 - Genesis Reward (Backfill)',
  'completed',
  json_build_object('backfill', true, 'is_genesis_reward', true)
FROM profiles WHERE full_name = 'Victor';

-- Vivian (Machine 2) - Credit genesis bonus
INSERT INTO transactions (user_id, amount, transaction_type, wallet_type, description, status, metadata)
SELECT 
  id,
  1000,
  'genesis_bonus',
  'deposit',
  'Genesis Complete! Bonus credit for Machine 2 (Backfill)',
  'completed',
  json_build_object('backfill', true, 'reason', 'Machine created without ledger entry')
FROM profiles WHERE full_name = 'Vivian';

-- Vivian (Machine 2) - Debit for machine purchase
INSERT INTO transactions (user_id, amount, transaction_type, wallet_type, description, status, metadata)
SELECT 
  id,
  -1000,
  'drop_entry',
  'deposit',
  'Bought Machine 2 - Genesis Reward (Backfill)',
  'completed',
  json_build_object('backfill', true, 'is_genesis_reward', true)
FROM profiles WHERE full_name = 'Vivian';

-- Update distribute_liquidity function with proper ledger accounting
  ...
  WHILE v_remaining > 0 AND v_depth < _max_depth LOOP
    ...
    IF v_target_drop.fill_amount + v_amount_to_fill >= v_target_drop.target_amount THEN
      ...
      IF v_is_genesis_spot AND v_genesis_remaining > 0 THEN
        ...
        -- ============================================
        -- GENESIS COMPLETION: When yields hit 0, create Machine 2
        -- ============================================
        IF v_genesis_remaining = 0 THEN
          ...
          -- Create the 2nd machine with PROPER LEDGER ENTRIES (only if they have exactly 1 spot)
          IF v_user_spot_count = 1 THEN
            -- Step 1: Credit genesis completion bonus to deposit wallet
            INSERT INTO transactions (
              ...
              'genesis_bonus',
              'deposit',
              'Genesis Complete! Bonus credit for Machine 2',
              'completed',
              json_build_object('genesis_spot_id', v_target_drop.spot_id)
            );
            
            -- Step 2: Debit to "buy" Machine 2
            INSERT INTO transactions (
              user_id, amount, transaction_type, wallet_type,
              description, status, metadata
            ) VALUES (
              ...
              'drop_entry',
              'deposit',
              'Bought Machine 2 - Genesis Reward',
              'completed',
              json_build_object('is_genesis_reward', true)
            );
            
            -- Step 3: Create new spot (Machine 2)
            ...
            -- Step 4: Get next position for the new machine
            SELECT get_next_drop_position() INTO v_new_genesis_position;
            
            -- Step 5: Create drop for the new spot
            ...
            -- Step 6: Invalidate cached balance
            DELETE FROM cached_balances WHERE user_id = v_spot_owner_id;
            
            -- Step 7: Send genesis complete notification
