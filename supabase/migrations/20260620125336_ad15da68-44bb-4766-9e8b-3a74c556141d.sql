...
-- Flat per-batch payout: each completed batch always pays exactly
-- task_naira_per_batch (e.g. ₦900), regardless of how many spots the user owns.
-- Previously the value was multiplied by the user's active spot count, which
-- broke the unlock-system math (₦900 × N batches → ₦27,000).
...
BEGIN
  ...
  -- Flat payout: every batch pays the same amount, no spot multiplier
