...
ALTER TABLE public.platform_config
  ADD COLUMN IF NOT EXISTS referral_pending_bonus numeric NOT NULL DEFAULT 3000;

-- 2. Switch task math to flat ₦50, unlimited batches
UPDATE public.platform_config
   SET task_naira_per_batch = 50,
       task_batches_per_day = 0,
       referral_pending_bonus = 3000
 WHERE id = 1;

-- 3. Rewrite complete_batch: flat rate, unlimited-mode aware
...
BEGIN
  ...
  -- FLAT rate: same ₦50 regardless of spot count. Extra spots just extend
  -- the empire capacity ceiling, they don't multiply per-batch yield.
  ...
  -- Capacity cap: pending must not overflow ₦10k × active spots
  ...
  -- Daily limit only applies when NOT unlimited
  IF NOT unlimited THEN
    ...
    END IF;
  ELSE
    total_today := 0; -- 0 = unlimited sentinel for clients
  END IF;
  ...
  -- First-full-set referral bump only meaningful in limited mode. In
  -- unlimited mode the ₦3k referral pending bump replaces this reward.
...
-- 4. Rewrite get_daily_task: flat rate, unlimited flag
...
BEGIN
  ...
  SELECT task_batches_per_day, task_taps_per_batch, task_naira_per_batch,
         ...
         referral_cash_bonus, referral_pending_bonus, drop_profit_amount, drop_entry_fee
    INTO cfg FROM public.platform_config WHERE id = 1;
...
-- 5. Rewrite pay_referrer_activation_bonus: cash + pending-balance jump
...
BEGIN
  ...
  SELECT referral_cash_bonus, referral_pending_bonus, drop_profit_amount
    INTO v_cash_amount, v_pending_bump_configured, v_payout_per_spot
    FROM platform_config WHERE id = 1;

  SELECT full_name INTO v_referee_name FROM profiles WHERE id = _referee_id;

  -- ============ Part A: cash bonus (idempotent) ============
  ...
  -- ============ Part B: pending-balance jump (idempotent) ============
  -- Only applies if the referrer has active spots AND still has capacity room.
  -- Cap the bump so pending can never exceed drop_profit_amount × spot_count.
  IF v_pending_bump_configured IS NOT NULL AND v_pending_bump_configured > 0 THEN
    ...
    IF NOT v_already_bumped THEN
      ...
      IF v_referrer_spots > 0 THEN
        v_referrer_cap := v_payout_per_spot * v_referrer_spots;
  ...
  -- ============ Part C: notification (only when something was credited) ============
