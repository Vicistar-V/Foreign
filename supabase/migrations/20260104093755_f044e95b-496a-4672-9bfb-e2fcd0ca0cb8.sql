-- Update atomic_play_instant to include "Safe Mode" for first 2 plays
-- First 2 plays are ALWAYS refunds (costs nothing, builds trust)
...
BEGIN
  ...
  -- =====================================================
  -- STEP 3.5: SAFE MODE - First 2 plays are ALWAYS refunds
  -- This builds trust without costing money (credits cycle back)
  -- =====================================================
  SELECT COUNT(*) INTO v_user_play_count
  FROM public.instant_play_history
  WHERE user_id = _user_id;
  
  -- First 2 plays are "Safe Mode" - forced refunds
  IF v_user_play_count < 2 THEN
    v_safe_mode_active := TRUE;
    
    -- Pick a REFUND outcome specifically (not random)
    ...
    -- If no refund available, fall back to any non-loss outcome
